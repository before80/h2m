...
func findATextAndHref(ulLen int, level int, liSel *goquery.Selection) (string, string) {
	fmt.Println("urlLen=", ulLen, level, liSel.Text())
	var aSel *goquery.Selection
	if ulLen == 0 {
		aSel = liSel.ChildrenFiltered("a").First()
	} else {
		aSel = liSel.ChildrenFiltered("div").First().ChildrenFiltered("a").First()
	}
...
func httpGetContentAndWriteToHtmlFile(cmd *cobra.Command, link, newFilePathWithoutExt string) {
	...
	//
	////fmt.Println("url=", url)
	////if sels.Find("#subnav").Size() != 0 {
	////	fmt.Println("1 found subnav")
	////	sels.RemoveFiltered("#subnav")
	////}
	////
	////if sels.Find("#page-github-links").Size() != 0 {
	////	fmt.Println("found page-github-links")
	////	sels.RemoveFiltered("#page-github-links")
	////}
	////
	////if sels.Find("#subnav").Size() != 0 {
	////	fmt.Println("2 found subnav")
	////	sels.RemoveFiltered("#subnav")
	////}
	//
...
func Html2md(cmd *cobra.Command, args []string) {
	...
	liSelX0s.Each(func(liIndex0 int, liSel0 *goquery.Selection) {
		ulSelX1s := liSel0.ChildrenFiltered("ul")
		// 若第一层菜单下没有找到 ul 标签，则说明该菜单下没有子菜单
		if ulSelX1s.Length() == 0 {
			menuText0, menuLink0 = findATextAndHref(0, 0, liSel0)
			...
			menuText0, menuLink0 = findATextAndHref(1, 0, liSel0)
			dirName0 = fileNameFromMenuText(menuText0)
			fileName0 := "_index"
			createDirAndMdFileAndWrite(withSlashJoinStr(dist, dirName0), fileName0, genNonIndexHeadStr(menuText0, baseUrl, menuLink0, liIndex0))
			...
			ulSelX1s.Each(func(ulIndex1 int, ulSel1 *goquery.Selection) {
				...
				liSelX1s.Each(func(liIndex1 int, liSel1 *goquery.Selection) {
					// ------------------------------------------------------------------------------------
					ulSelX2s := liSel1.ChildrenFiltered("ul")
					// ------------------------------------------------------------------------------------
					if ulSelX2s.Length() == 0 {
						menuText1, menuLink1 = findATextAndHref(0, 1, liSel1)
						...
						menuText1, menuLink1 = findATextAndHref(1, 1, liSel1)
						...
						ulSelX2s.Each(func(ulIndex2 int, ulSel2 *goquery.Selection) {
							...
							liSelX2s.Each(func(liIndex2 int, liSel2 *goquery.Selection) {
								// ------------------------------------------------------------------------------------
								ulSelX3s := liSel2.ChildrenFiltered("ul")
								// ------------------------------------------------------------------------------------
								if ulSelX3s.Length() == 0 {
									menuText2, menuLink2 = findATextAndHref(0, 2, liSel2)
									...
									menuText2, menuLink2 = findATextAndHref(1, 2, liSel2)
									...
									ulSelX3s.Each(func(ulIndex3 int, ulSel3 *goquery.Selection) {
										...
										liSelX3s.Each(func(liIndex3 int, liSel3 *goquery.Selection) {
											// ------------------------------------------------------------------------------------
											ulSelX4s := liSel3.ChildrenFiltered("ul")
											// ------------------------------------------------------------------------------------
											if ulSelX4s.Length() == 0 {
												menuText3, menuLink3 = findATextAndHref(0, 3, liSel3)
												...
												menuText3, menuLink3 = findATextAndHref(1, 3, liSel3)
												...
												ulSelX4s.Each(func(ulIndex4 int, ulSel4 *goquery.Selection) {
													...
													liSelX4s.Each(func(liIndex4 int, liSel4 *goquery.Selection) {
														// ------------------------------------------------------------------------------------
														ulSelX5s := liSel4.ChildrenFiltered("ul")
														// ------------------------------------------------------------------------------------
														if ulSelX5s.Length() == 0 {
															menuText4, menuLink4 = findATextAndHref(0, 4, liSel4)
															...
															menuText4, menuLink4 = findATextAndHref(1, 4, liSel4)
															...
															ulSelX5s.Each(func(ulIndex5 int, ulSel5 *goquery.Selection) {
																...
																liSelX5s.Each(func(liIndex5 int, liSel5 *goquery.Selection) {
																	// ------------------------------------------------------------------------------------
																	ulSelX6s := liSel5.ChildrenFiltered("ul")
																	// ------------------------------------------------------------------------------------
																	if ulSelX6s.Length() == 0 {
																		menuText5, menuLink5 = findATextAndHref(0, 5, liSel5)
